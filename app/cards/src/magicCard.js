...
function drawTypeBar(ctx) {
  ...
  ctx.textBaseline = 'middle';
  ctx.textAlign = 'left';
  ctx.fillText('Artefatto', x + 22, y + h / 2 + 1);
...
function drawBottomInfo(ctx) {
  ...
  ctx.fillText('Illus. — generata in canvas', W / 2, y);
...
function drawTapSymbol(ctx, cx, cy, r) {
  ...
  const bh = r * 0.95;
  ctx.beginPath();
  ctx.rect(-bw / 2, -bh * 0.5, bw, bh * 0.85);
  ctx.fill();
  ctx.beginPath();
  ctx.moveTo(-r * 0.45, bh * 0.32);
  ctx.lineTo(0, bh * 0.55);
  ctx.lineTo(r * 0.45, bh * 0.32);
  ctx.lineTo(r * 0.18, bh * 0.32);
  ctx.lineTo(r * 0.18, bh * 0.18);
  ctx.lineTo(-r * 0.18, bh * 0.18);
  ctx.lineTo(-r * 0.18, bh * 0.32);
  ctx.closePath();
  ctx.fill();
